...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n`MadBoi's Bot is`  **up-to-date**  `with`  "
    ...
    if conf == "" and not force_update:
        ...
        await event.delete()
        return await event.respond("Do `.update deploy` to update the MadBoi's Bot..")
...
async def upstream(event):
    event = await edit_or_reply(event, "`Pulling the Omki repo wait a sec ....`")
    off_repo = "https://github.com/madboy482/Omki"
    os.chdir("/app")
    catcmd = f"rm -rf .git"
    try:
...
@bot.on(admin_cmd(pattern=r"madboi$", outgoing=True))
@bot.on(sudo_cmd(pattern=r"madboi$", allow_sudo=True))
async def variable(var):
    ...
    await edit_or_reply(var, f"`Changing current version to MadBoi's Bot, wait for 2-3 minutes.`")
    heroku_var["UPSTREAM_REPO"] = "https://github.com/madboy482/Omk"
...
CMD_HELP.update(
    {
        ...
        "\n\n📌** CMD ➥** `madboi`"
        "\n**USAGE   ➥  **Shifts to official MadBoi repo from custom repo."
    }
